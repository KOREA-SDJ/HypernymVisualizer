"""CLIP 모델 관리 및 임베딩 생성 모듈.

이 모듈은 Hugging Face Transformers 라이브러리를 사용하여 사전 학습된 CLIP 모델을 로드하고,
텍스트와 이미지 데이터를 정규화된 벡터 임베딩으로 변환하는 기능을 제공합니다.
모델 로딩 시 싱글톤 패턴을 적용하여 중복 메모리 사용을 방지합니다.
"""
...
def load_clip_model(model_name: str = "openai/clip-vit-base-patch32") -> Tuple[CLIPProcessor, CLIPModel]:
    ...
    지정된 장치(CUDA 또는 CPU)에 모델을 로드합니다.
    전역 변수를 활용한 싱글톤 패턴을 사용하여, 함수가 여러 번 호출되더라도
    모델을 메모리에 중복으로 로드하지 않도록 보장합니다.

    Args:
        model_name (str, optional): Hugging Face Hub 모델 식별자.
            기본값은 "openai/clip-vit-base-patch32"입니다.

    Returns:
        Tuple[CLIPProcessor, CLIPModel]: 
            - processor: 텍스트 토큰화 및 이미지 전처리 객체.
            - model: 로드된 CLIP 모델 객체.

    Raises:
        OSError: 모델 이름을 찾을 수 없거나 네트워크 오류 발생 시.
        Exception: 그 외 모델 로딩 중 치명적인 오류 발생 시.
    """
...
def get_text_features(
    ...
    """텍스트 리스트에 대한 정규화된 CLIP 임베딩을 생성합니다.
    ...
    Returns:
        torch.Tensor: (N, D) 크기의 정규화된 임베딩 텐서.
            - N: 입력 텍스트 개수
            - D: 임베딩 차원
    ...
    # L2 정규화 (Cosine Similarity 계산을 위함)
...
def get_image_features(
    ...
    """이미지 리스트에 대한 정규화된 CLIP 임베딩을 생성합니다.

    Args:
        images (List[Image.Image]): 임베딩할 PIL 이미지 리스트.
        processor (CLIPProcessor): 로드된 CLIP 프로세서.
        model (CLIPModel): 로드된 CLIP 모델.
...
def calculate_similarity(
    ...
    """텍스트와 이미지 임베딩 간의 코사인 유사도 행렬을 계산합니다.

    입력된 임베딩들이 이미 정규화되어 있으므로, 벡터의 내적(Dot Product)이
    곧 코사인 유사도와 같습니다.

    Args:
        text_features (torch.Tensor): (M, D) 크기의 텍스트 임베딩.
        image_features (torch.Tensor): (N, D) 크기의 이미지 임베딩.

    Returns:
        torch.Tensor: (N, M) 크기의 유사도 행렬 (CPU 텐서).
            - N: 이미지 개수
            - M: 텍스트 개수
